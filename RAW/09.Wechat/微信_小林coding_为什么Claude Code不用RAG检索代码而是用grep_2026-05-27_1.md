---
title: "微信_小林coding_公众号文章剪藏_2026-05-27_1"
...
## 0x01. 字节面试官：“为什么Claude Code不用RAG检索代码，而是用grep？”我："因为...省钱?" 他沉默了三秒

> 发布日期：2026-05-16  
> 原文链接：[字节面试官：“为什么Claude Code不用RAG检索代码，而是用grep？”我："因为...省钱?" 他沉默了三秒](https://mp.weixin.qq.com/s/Q2RUOGymelU9F5LsLdR4UA)
...
![image](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX05UEmnfQ27vIDdZo3TpOCj6AiaCXiaLkcniaQZjicicECxS4vrphWnSuz5xkPCIMh6hbtOfMmRJNmxtFFBicdTO6a2R0jwib6XDnGd6k/640?wx_fmt=png&from=appmsg)
 我一愣。
...
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX2pm12aVLRla6GzlNLg5br26Ds2ua74JiaUX5ibPBDXykyYTW8nTtVAF0ibI50dFuL3axxt8f4JovoKx48Bh3ohJ9LpAKt3PR0oOg/640?wx_fmt=png&from=appmsg)
...
### 1. 先搞懂「代码检索」到底要解决什么问题
 ...
 塞不下，还是塞不下。
![上下文窗口 vs 代码库体积对比图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX0n4U15icOMFsibms6xcVVc2sJvBkOOf6QIibwobt0ibRvbpicxbUaIjDAiaH6o01icHZicLJYibDKAV9V0B1xrLkkV36NZObWpSvxrv48k/640?wx_fmt=png&from=appmsg)
 上下文窗口 vs 代码库体积对比图
...
![字典查目录类比图](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX2JSvcicoRvPxYvxwSsXPaBjZK3VcDva7TQyibrpU7hsVX3wIPTpsn7xMJl6nmiaRy3xVsOws0tlGffSzHryBA3MD2iabibC0BicoicW0/640?wx_fmt=png&from=appmsg)
 字典查目录类比图
 那「怎么查目录」、「怎么定位到那一页」，就是不同方案的差异所在了。

### 2. 绕不开的 RAG：「先建库再查」的经典思路
 ...
 **第一步，切片(Chunking)。** 先把代码文件按某种规则切成小片段。常见的切法是按函数切、按类切，或者按固定行数切，比如每 100 行一段。这相当于把厚厚的书拆成一篇篇文章。
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX1ADicYbbhym7wuianficyX4D7skuXauibJiaic3Jrt7BicoWI2OVwUSN7ZSjYx97hgqqdnoCUB0r6ubiaB1pA3BEv7007eZVafLQ8M5U4/640?wx_fmt=png&from=appmsg)
 **第二步，向量化(Embedding)。** 每个代码片段过一遍 embedding 模型，转成一串数字(一个高维向量)。你可以理解成给每个片段算了一个「语义指纹」，意思相近的片段，指纹也相近。
![Embedding 演示：代码经过模型变成数字向量](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX2GYp7YzTI2hhOBicQ4dOeNwprQ1klANNTgiaDJ7JejbAySwIEiawckibCaLdLebf4Hve4X7gS7R0ADXU6VS5OCpoLhxl5A3Ne29eM/640?wx_fmt=png&from=appmsg)
 Embedding 演示：代码经过模型变成数字向量
 **第三步，建索引(Indexing)。** 把所有的向量存到一个专门的向量数据库里，比如 Faiss、Pinecone、Milvus 这些。这一步就是「把分类卡片整理好，按抽屉摆放」。

 **第四步，召回(Retrieval)。** 用户提问的时候，把问题也向量化一下，去库里找最相似的 Top-K 个片段，比如最像的 5 个，然后把这几段代码拼到 prompt 里，丢给 LLM。
![向量召回 Top-K 示意图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX0ibS6kENEWDfHm4LicA0iaTWBdA57VGaFtcFSWiaxuj5Q1xH9k7zxiaE0NTiaedxG32Pk0Lezy9zoXicricHFxkVaquXFpFXc8MEeVuZs/640?wx_fmt=png&from=appmsg)
 向量召回 Top-K 示意图
...
![函数被切两半示意图](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX3ibF5uI33sXoC4F1ibR3zfnNibpBk15MRPo2uQUqEkEzk0dMQP84fKN28XcuMRWfdUuicgCicM1xibP1cXTiaOP6ojJvibUSmPc4IfhXY/640?wx_fmt=png&from=appmsg)
 函数被切两半示意图
#### 痛点 2：精确匹配的活，向量干不了
...
![向量召回错误示意图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX1PDsZXvJjEnqNoQaEtUx40X2BYwKJ606u9eb1aCSOt7oIOoDguLhm4UZamy7yrTxnYdNiaPxQn7eibAwcCZia2t0HredQlY6fYv4/640?wx_fmt=png&from=appmsg)
 向量召回错误示意图
#### 痛点 3：代码每天在变，索引咋办
 这是 RAG 在代码场景的另一个噩梦。
...
![索引重建成本图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX3lRsa11P1r6X9XfNRsqliaot03wdvHfRkQrrJFPQqPNmqcGJaJqicicASNnY4ujP8tO3No6tANVAJxc2Kuj8BvSyZxgWxfhWqp3A/640?wx_fmt=png&from=appmsg)
 索引重建成本图
#### 痛点 4：冷启动慢得要命
 你在一个百万行的代码库上跑 RAG，光是建索引就要十几分钟甚至更久。

 用户打开工具，看着进度条转，等几分钟才能开始用？这体验直接劝退一半人。Claude Code 的设计理念是「打开就能用」，RAG 这套显然不答应。
![冷启动等待图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX1EuHGsGbb5dmJf3wcPN55IjOo8RFxu0LhCBooZIhV7tOM8I46HsR2U3FGYQxgfEkUFNxYIdNLXmDDo7XFa2oj4HfawxnUpnMs/640?wx_fmt=png&from=appmsg)
 冷启动等待图
#### 痛点 5：黑盒，不可解释
 这点我觉得最关键。
...
### 4. Claude Code 的反向思路：把检索还给模型自己
 ...
 简单到让人意外： **让模型像程序员一样，自己去找。** 不预处理、不建库、不算向量，每次需要代码就实时去现场查。工具就给三个：
...
![程序员工作流与 Claude Code 三件套对应关系](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX0x42BqbcslS2MWMFBJkSatoaD6vAvpqr8ibky7x76H9ZIIGVrrdv1k7wlMqj8kyL6JFLZ5ZeHibuMJXQWFl9iaapeiaTnxxmGoAfo/640?wx_fmt=png&from=appmsg)
 ...
 **第一层，权限统一管控。** Bash 是个万能工具，模型理论上能跑任何命令。如果让它自己跑 grep，那 rm 也能跑、curl 也能跑、git push 也能跑。Claude Code 把 grep 单独包成工具，相当于在这个高频操作上单独画了一道权限闸门，更安全。

 **第二层，输出格式可控。** 你直接跑 bash grep，输出就是一坨纯文本。但 Grep 工具可以提供结构化输出：行号、上下文行、按文件分组、甚至支持「只返回匹配文件名」、「只返回匹配数量」三种粒度。模型按需选择，token 浪费少很多。

 **第三层，性能。** Claude Code 的 Grep 底层用的是 ripgrep，不是传统 grep。ripgrep 是 Rust 写的，多线程并行、自动尊重 .gitignore(不去搜 node_modules 这种垃圾目录)，性能甩老牌 grep 几条街。
...
![Grep 三种输出模式示意图](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX3q3o2uQ1QqwTbNehUFfj36qWwcic82F4tr287cBBI4EYGVV9IqeVlN4vD520eQJeR4Y7XTziaUrvUtqfILzVGA2FsRC3O7dGjH0/640?wx_fmt=png&from=appmsg)
 Grep 三种输出模式示意图
...
![Glob pattern 示意图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX29ZELT5LxdmozYDKybEDIRk6RlNX4TWzmFHQx8FHGZPZ4I7VtEyI39dtKdDibHZZrpraAnibPMSyYPwjwHPX2m16LV5Eb7kpyKA/640?wx_fmt=png&from=appmsg)
 Glob pattern 示意图
#### 5.3 Read：按需读取，绝不贪心
...
![磁盘实时读取 vs 索引缓存对比图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX24VHQy7ndkWoXBficUWzwibbgLcB8icUSsiaN8lYMCmAicic6dLYU5hRfO7qvDL8SPk9ibmEg0MOmfP0lvXXqiaE24AEtHoDo4WPkniaog/640?wx_fmt=png&from=appmsg)
 磁盘实时读取 vs 索引缓存对比图
#### 5.4 三件套的组合用法
 讲完单个工具，最关键的是看它们怎么组合。
...
![Glob Grep Read 三件套组合工作流](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX24jZbRITDPatibibdKZtHj46MtZadOiaRS4CDz82jPNomqH8kKVo6OyyhskvmiaOHMUqLicL2L8iak4qXRia4ia8MtgiatM2RVKgv3T8P4/640?wx_fmt=png&from=appmsg)
...
![主 agent 上下文被污染示意图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX0icicrHOTOATZ3v8hWPPXIsp4AmSglQOetz4WJ2Af5oAh2hyjBxHibMUoje5PFV2ytnldzotPdjeMKbXIlPBxDgCKNAnuOkibFNps/640?wx_fmt=png&from=appmsg)
 主 agent 上下文被污染示意图
...
![派子 agent 隔离与结论压缩示意图](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX0LkSOqm2P6eo1eWzbtFd4af2kGUSpHN3ORbOeUSsvJSU5F1ibkVOBcic5pxUUhHIEKBqWJ5ow9hkVWZlxWk65GiaFebTwoInqicOA/640?wx_fmt=png&from=appmsg)
 派子 agent 隔离与结论压缩示意图
...
![并行派多个 Explore 子 agent 示意图](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX0mq4ibiaOW34fq1qGibAqRzhPASXFLJicpiccb1nyHuPbnRw0UO4Wf6qiaMk1HIcef5P3YIIx0MxzuuE8veoSCQVpoxHhmSedBTSve8/640?wx_fmt=png&from=appmsg)
...
![RAG 一锤子 vs Agent 多轮循环对比图](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX2PwE27y5gLFsZmzZibiaaUIKQpdFxveklJ9Bu749W9p2jWrfHpgMEMPrpRFHTHUc3lniajVWPpwd2xyXOwuQDsgE5whRVF98I1YI/640?wx_fmt=png&from=appmsg)
...
![query 多轮循环示意图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX0M9zMbkH4BKicicG1iboYswfjMv2pa9TkgygibsbmH89vg7N3TicCiab8eRSFMFHsRp1YfkzPicI2XGwibfu72q11ia1ukibFxhqYftHhps/640?wx_fmt=png&from=appmsg)
 query 多轮循环示意图
...
### 8. 回到原题：到底为什么 Claude Code 不用 RAG？
...
![Claude Code grep 与 RAG 六大原因对比表](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX3uzDITW7leIdxFOXZIjlniaJajOPNJXmwH91RibuJNrSnGVP0C5aHMc8aiaC4euw9PT9yibrmYuzzO3GXN1A2Axoazb7m1CpgyLB4/640?wx_fmt=png&from=appmsg)
...
![RAG 与 Claude Code 设计哲学对比图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX1ibju02KhOoHTxTfZOwT6D0Wwh6VzHH5LHb8nsicNtIuwiahaiaGPmhiarfyxbQxWpiaHHWml1t3o3EcXMSG5F7F7fgWMyFsPE09m6Y/640?wx_fmt=png&from=appmsg)
...
![grep 与 RAG 场景适配对比图](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX3icLr4ybkt8hrABwibYv3S1iawKic0GslBXeIVMnicgBQxictoWn6SRA7Xbsla6uz0ywhYYLWmhJ9xDXF2BkITWQWu36icAyCfNjFLHM/640?wx_fmt=png&from=appmsg)
 grep 与 RAG 场景适配对比图
### 结尾：回到面试题
...
![image](https://mmbiz.qpic.cn/mmbiz_png/ysyAxM1rgX3nFTTsY4Dr5EqlE8gJB5WkDAJLbpABcTCIK28aXsNQ1egus1bdAecdec28dZL8drDKMLQXVDP75DRcWW5rJgaHHrgbqGQQEt0/640?wx_fmt=png&from=appmsg)

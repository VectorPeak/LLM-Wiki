---
title: "微信_放之_公众号文章剪藏_2026-05-26_1-2"
...
> 发布日期：2026-03-10  
> 原文链接：[Vibe Coding生存指南](https://mp.weixin.qq.com/s/s9dTiR4okEPHB2xnjv8B5A)
 ...
 **84亿Token、38948条消息、58个项目、547次Cursor对话、204次Claude Code会话。** 三个核心项目的对话量：
...
| 高 | 140 | 6 | 1 | 0 | 147 | 4.8% |
![img](https://mmbiz.qpic.cn/mmbiz_gif/9UxrUvqHyjzxp2ibvw5XdIKemVUnbt0EFbrDn213RP7IIyCyExH9tQgF8L0KvnHNniczichjtTU0P4Uz1CX8U7MXX9aHX42SwuzY7RphjbBX4M/640?wx_fmt=gif&from=appmsg)
...
![img](https://mmbiz.qpic.cn/mmbiz_gif/9UxrUvqHyjw53UgLN4icnebOzw5XpTzzZOIHtPaZ6FPQufR95wprEzLLcuysn9bzr0OAjEvuHX9GAa3kBgfpKWeAF1Bgu8wibdRRBuXLM3Ys8/640?wx_fmt=gif&from=appmsg)
...
![image.png](https://img.vectorpeak.cn/obsidian/2026/05-06/20260527091331030.png?imageSlim)

图注：X轴为每条消息产出的代码行数（效率），Y轴为成功率。大项目（紫色圆圈）反而多落在右上角的“高效区”。

为什么右上角那些高产出、高成功率的“大圆圈”（如 AgenticaSoC），全是高复杂度任务？

因为当一个任务被标记为“高复杂度”时，你会主动启用全套工程规范：老老实实地写PRD、分配Task ID、挂载自动化测试、做 Git Checkpoint。
...
![img](https://mmbiz.qpic.cn/sz_mmbiz_png/9UxrUvqHyjzaYAfAFNSeMXaDOReMJVIjClLvjLNibyOKBYiaRPJAQ6S3euyK3LqOm2n6h6ydTjwCdex3M6mxgm5YrPicmjVZOdHfwjXfM7L7bI/640?wx_fmt=png&from=appmsg)
...
![img](https://mmbiz.qpic.cn/sz_mmbiz_png/9UxrUvqHyjzy3cqbagXFiccSpugibjFLSuMPjYmgtvtticxV3DicnKsl5khhwjAOLbUhtYcgx8hG0LGv35T7YnvpJcmxQ6tRvJrl4kjlQ2NozHY/640?wx_fmt=png&from=appmsg)
...
![img](https://mmbiz.qpic.cn/sz_mmbiz_png/9UxrUvqHyjxaxpFIicicIST2SicTZC82RjpwxgMQ8neRxwS7CSYia6C3IqcJxAwxZSH3WK9PKZ6CpNLGGYib8jFr1l1yCVjiaJveEria6hwXafAH5k/640?wx_fmt=png&from=appmsg)
 *图注：短闭环(Cursor主导)呈现高耸的集中峰值，长循环(Claude主导)拖出了一条沉重的 “Fat Tail”。*
![img](https://mmbiz.qpic.cn/sz_mmbiz_png/9UxrUvqHyjyChKu9JGOeibOZOEic6RqQ2CmPHibWvpDRKPIAw6HibV7G7oAzjn7H8B0FwRBgDR6ROxQopxfribwM883jMu6mHO5CwMoC2qlMarnk/640?wx_fmt=png&from=appmsg)
...
![img](https://mmbiz.qpic.cn/mmbiz_gif/9UxrUvqHyjxB2EYZtZZljgFjaa9NzEf6Ee5H8uFTuVgyhdHK2rA0awxyqspNvLYibQ4UTWLxy05eJNV3BYxS46rZqWaeWe6GXJUylw7yddCM/640?wx_fmt=gif&from=appmsg)
...
##### 案例一：Agent守卫(Guardrails)的错误抽象
 ...
 结果：Claude Opus 在处理复杂任务时，合理的多步工具调用(比如依次调用 read_file 读三个不同文件)被误判为”死循环”，任务被提前终止。 **AI 不是在卡死，是熔断器设计得太粗糙。** 修复后的分层方案：
...
![img](https://mmbiz.qpic.cn/mmbiz_png/9UxrUvqHyjxuRCKwJR1yS1HqKgE8XaxmtDmRoZCxjwNKmwibWYazjDr8ZKW8zzkptCFjBBkCACeKIMdZaBbQOdZvXRcrShYLrADria5wbAmpU/640?wx_fmt=png&from=appmsg)
...
![img](https://mmbiz.qpic.cn/sz_mmbiz_png/9UxrUvqHyjw6T1PmwOrw0wUdXBZ5gZK505jdYRo0PR6ZVAqV47v0dDiaaWfEicuBZoPCwtFLRulBRxwe0BEVela6A9yrUf3ZGT3icz4055DUP4/640?wx_fmt=png&from=appmsg)
...
![img](https://mmbiz.qpic.cn/sz_mmbiz_png/9UxrUvqHyjxwjrFbjvziafA1a1YPnpr2AtBlcw9hWjdH7UTia4YZicHypuqRBErtkXzsyl1cpzibrewJTeeZNIb2TfTCiagR5HWBhe1lqomMuHYE/640?wx_fmt=png&from=appmsg)
 ...
 中上：项目规模 vs 会话深度(气泡图)
	...
	核心结论：大项目并不天然失败；失败通常发生在”高消息数 + 低结构化管理”的组![img](https://mmbiz.qpic.cn/mmbiz_gif/9UxrUvqHyjyYvLVZwnFF5e8XI48miajsT0hZn1gCmnQ6EnKOfHxicMk1ndib1ibDSQbT1eicwEG2FHGv2mOmUTHvlpoRdQfhrc73AgVElCHx1zico/640?wx_fmt=gif&from=appmsg)合。
![img](https://mmbiz.qpic.cn/mmbiz_gif/9UxrUvqHyjyYvLVZwnFF5e8XI48miajsT0hZn1gCmnQ6EnKOfHxicMk1ndib1ibDSQbT1eicwEG2FHGv2mOmUTHvlpoRdQfhrc73AgVElCHx1zico/640?wx_fmt=gif&from=appmsg)
 >这个动画展示了项目是如何落入”高效区(绿色)”或”迭代区(黄色)”的。小项目(绿色菱形)天然落在高效区，成功率 100%。大项目(紫色圆形)如果管理不当，就会掉进迭代区，成功率跌至 67%。

 右上：消息量密度分布(按结果)
	不同结果类型(成功/部分/失败/放弃)的会话长度分布。
	核心结论：成功会话集中在短闭环(20-50条消息)，失败会话拖出长尾(200+条消息)。![img](https://mmbiz.qpic.cn/mmbiz_gif/9UxrUvqHyjysfmkZSMaQVbR2ibXKMaEwB9OicaTHBeFcJ5ENoZ4TbZ0bfFEXjAfeoV5V0jaQqYVk85yn2zM31tfxALmVIJflhdMutjfaHVems/640?wx_fmt=gif&from=appmsg)
 ...
 >这张图说明：Vibe Coding 的关键不仅是模型更强，还有更重要的会话边界管理。 会话保持短闭环，成功率很高；当会话跨过上下文临界点，失败会迅速累积。所以真正有效的方法是： **小步快跑、及时 checkpoint、到边界就重开会话。** ### 0x08 终极战术手册：DOs and DON’Ts
...
```
#### DO
 - 用 Plan-then-Do 提示强制 AI 先思考再写代码 "Role: Senior Architect. 基于 @prd.json 生成分步实现计划，JSON格式输出，禁止使用占位符"
 - **每次重要改动前 Git Checkpoint**
 - **3–5 次连续失败后触发熔断器 不要让 AI 无限重试，人工介入重置上下文**
 - **“接上或删掉”规则 每个新生成的工具模块必须在当前会话结束前至少有一个调用者。不允许孤儿文件。**
 - **用绝对导入替代相对导入 深层目录结构里，相对导入是沙堡，一次重构全部崩塌**
 - **单会话一致性检查 会话结束前验证：新文件是否遵循现有模式？是否和 style rules 矛盾？**
 - **“给我看调用者”规则 当 AI 生成一个 utility/module，立即问：”现在给我看现有代码中谁会调用它。”**
 - **多租户类架构从第一行代码开始设计 后期改造 tenant_id 是手术，早期设计是疫苗**
 - **Feature Flag 审计节奏 审查所有 flag。如果一个 flag 已经 False 超过两周，要么启用它，要么删除这个功能。**
 - **限制 AI 单次变更 < 500 行 如果重构需要更多，拆成多个阶段，每个阶段有可工作的中间状态。(人工Review的情况下，可以更多一些)**

#### DON’T
 - **永远不要 except Exception: pass 这条 bug 修了三个月，因为它让所有错误无声消失**
 - **不要 hardcode localhost 或相对路径 容器化的那天你会哭着想起这条**
 - **核心文件不要超过 1000 行 强制执行 500 行限制重构，超过 1000 行就是技术债墙**
 - **不要在 LangGraph 里混用 sync 节点和 async LLM 调用 async/sync 嵌合体——第三宗罪**
 - **不要接受 Placeholder // TODO: implement 在早期看起来无害，在后期是地雷阵。AI 生成的占位符更危险。**
 - **不要用情绪化语言给 AI 施压 "你TM的为什么又搞错了！！！" 不提供技术上下文，只消耗 Token。AI又不是你的出气筒，正确姿势：提供 stack trace + 请求”Root Cause Analysis”。**
 - **不要在长会话里没有 /compact 就继续走 200+条消息后的失败率是 35.7%，是正常区间的5倍**
 - **不要信任”Phase Complete”文档 验证实际状态，而不是文档声称的百分比。**
 - **不要做大爆炸式重写**
...
| 大项目 + 无结构 + 自主运行 | ~55% | 什么都不适合——这是陷阱 | 极高 |
 ...
 好消息是：知道临界点在哪里，本身就是力量。 在临界点以下，尽情享受 Vibe Coding 的魔法。在临界点以上，老老实实切换到架构师模式——这不是退步，这是你的项目配得上的管理方式。 **别总想着走捷径，捷径往往是最远的路。** Vibe Coding 的终点，不是更快的代码生成，而是你成为一个用自然语言管理 AI 工程团队的架构师——一个明白”接上或删掉”、”增量不爆炸”、”结构即速度”的架构师。这是84亿 Token 和 价值约1W$的费用告诉我的，希望这笔昂贵的学费，能帮你少走弯路。

## 二. 多Agent协作时代的Git规范

> 发布日期：2026-05-09  
> 原文链接：[多Agent协作时代的Git规范](https://mp.weixin.qq.com/s/0NbH7Sd6-Nx4o8bMYX5jGw)

这不是Git入门教程。这是团队协作，单人多Agent在产品迭代时总结出来的一套”防互相踩踏”协作规约。文章系AI共同创作。
![image](https://mmbiz.qpic.cn/sz_mmbiz_png/9UxrUvqHyjyElu7iaCibsrZ2vUv6Aicl3NsvysQYbmUTKz0t0IN0WibGgicGx2M4icb7O49mrWHLP4TKSdicl6rH8PCPjUq6CJX4OdMwC9SSxzzEQU/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/mmbiz_png/9UxrUvqHyjydkIwx2ic1VFq5lBdHVvCDBaI82ic5t5ZDGlulNS8hR5ZMJg8VNPbwfxyOTlcwrZvFTBsvam96Y8AmaFdINQGlQBquYGbNu0vUw/640?wx_fmt=png&from=appmsg)
 worktree 目录用`.git/info/exclude` 忽略掉，避免本地执行环境(venv、node_modules、临时缓存)跟着 commit 误提交

 分支命名要携带来源和意图——它在多人协作里是索引，不是装饰品：
![image](https://mmbiz.qpic.cn/mmbiz_png/9UxrUvqHyjwawIlaCsUhyndN9WicqjdKsxdiaoQl1QseNYOtzqZpf9sFjibjc9Z4Z1LHqVYNupPrWFNhh3G3NANda0iccL98nX1IjTDbH30bNibA/640?wx_fmt=png&from=appmsg)
...
![image](https://mmbiz.qpic.cn/mmbiz_png/9UxrUvqHyjzVtSib2s26yvAic2ibsec1O0F2Jib6jCmcntTjOqQe6tNBP8nqq8VWYXThC3LW3t4VV3e5Dy9LJLPPqrW7oTkibUrLHsNb4wt0RANY/640?wx_fmt=png&from=appmsg)
...
| `stash` / `reset` / `force push` 前 | 确认 owner、备份旧状态、人类确认 |

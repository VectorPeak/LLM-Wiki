...
if __name__ = "__main__":
	...
	print("当前文档中每个词对应的索引：\n", corpus_idx)
```
def build_vocab(): [](#__codelineno-1-2)# 数据集位置 [](#__codelineno-1-3)file_name = 'data/jaychou_lyrics.txt' [](#__codelineno-1-4)# 分词结果存储位置 [](#__codelineno-1-5)unique_words = [] [](#__codelineno-1-6)all_words = [] [](#__codelineno-1-7)# 遍历数据集中的每一行文本 [](#__codelineno-1-8)for line in open(file_name, 'r'): [](#__codelineno-1-9)# 使用jieba分词,分割结果是一个列表 [](#__codelineno-1-10)words = jieba.lcut(line) [](#__codelineno-1-11)# print(words) [](#__codelineno-1-12)# 所有的分词结果存储到all_sentences，其中包含重复的词组 [](#__codelineno-1-13)all_words.append(words) [](#__codelineno-1-14)# 遍历分词结果，去重后存储到unique_words [](#__codelineno-1-15)for word in words: [](#__codelineno-1-16)if word not in unique_words: [](#__codelineno-1-17)unique_words.append(word) [](#__codelineno-1-18)# 语料中词的数量 [](#__codelineno-1-19)word_count = len(unique_words) [](#__codelineno-1-20)# 词到索引映射 [](#__codelineno-1-21)word_to_index = {word: idx for idx, word in enumerate(unique_words)} [](#__codelineno-1-22)# 词表索引表示 [](#__codelineno-1-23)corpus_idx = [] [](#__codelineno-1-24)# 遍历每一行的分词结果 [](#__codelineno-1-25)for words in all_words: [](#__codelineno-1-26)temp = [] [](#__codelineno-1-27)# 获取每一行的词，并获取相应的索引 [](#__codelineno-1-28)for word in words: [](#__codelineno-1-29)temp.append(word_to_index[word]) [](#__codelineno-1-30)# 在每行词之间添加空格隔开 [](#__codelineno-1-31)temp.append(word_to_index[' ']) [](#__codelineno-1-32)# 获取当前文档中每个词对应的索引 [](#__codelineno-1-33)corpus_idx.extend(temp) [](#__codelineno-1-34)return unique_words, word_to_index, word_count, corpus_idx

...
3. Build a structured export by scanning the vault:
   ...
   **For each note in 02.wiki/**, extract:
...
4. Output format:
   ...
   **JSON** (default):
   ```json
   {
     ...
     "notes": [
       {
         "path": "02.wiki/entities/Eric Siu.md",
         "title": "Eric Siu",
         "type": "entity",
...
5. Append to the operation log: write `**HH:MM** - export | Vault snapshot exported (format, N notes)` to `Logs/YYYY-MM-DD.md` (create `Logs/` and the daily file if missing; see `.codex/references/log.md` for the log format)

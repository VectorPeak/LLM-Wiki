...
1. Gather candidate signals - list and grep exhaustively (see the anti-fabrication rule):
   - Ungraduated idea/concept notes in `02.wiki/concepts/` (status `captured` or `exploring`, not yet `graduated`).
   - Raw clues in `01.raw/01.Inbox/` and related material across `02.wiki/**`.
   - Open questions in active project notes (Open Questions sections, unresolved decisions).
   - Orphan research notes in `04.output/Research/` that no project links to.
...
5. Save the shortlist to `02.wiki/concepts/YYYY-MM-DD - discovery.md` (`type: synthesis`, tagged `[thinking, idea-discovery]`) with source links in frontmatter. Do NOT auto-graduate anything - this command only surfaces and ranks.
...
**Anti-fabrication:** Rank only real candidates found in the vault - never invent an idea, an open question, or a research note to pad the shortlist. Enumerate `02.wiki/concepts/`, `01.raw/01.Inbox/`, `02.wiki/**`, project Open Questions, and orphan research exhaustively rather than sampling. See the anti-fabrication and search-completeness hard rules in `.codex/references/ai-first-rules.md`.

...
2. Read `index.md` and recent operation log for vault context (read the last 2-3 `Logs/YYYY-MM-DD.md` files; if no daily log exists yet, use `.codex/references/log.md` for the log format)
...
3. Spawn parallel subagents to gather learnings:
   ...
   - **Decisions agent**: read all ADRs in `02.wiki/decisions/` - extract the rationale and outcome of each
   - **Reports agent**: read recent emerge/synthesize/connect/challenge reports in `02.wiki/concepts/` (the auto-generated pattern reports)
...
6. Save the report to `02.wiki/concepts/YYYY-MM-DD - Learnings Review.md`
7. Append to the operation log: write `**HH:MM** - learn | X active, Y stale, Z superseded, N promotion candidates` to `Logs/YYYY-MM-DD.md` (create `Logs/` and the daily file if missing; see `.codex/references/log.md` for the log format)

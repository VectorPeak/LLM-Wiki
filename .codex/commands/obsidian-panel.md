...
2. Choose the panel:
   - If the vault has a `02.wiki/entities/advisors/` folder, read those persona notes and use each as a panelist (each verdict reflects that advisor's stated lens and priorities).
...
5. Save to `02.wiki/concepts/YYYY-MM-DD - panel - <slug>.md` (`type: synthesis`, tagged `[thinking, panel]`), linking any `[[02.wiki/entities/advisors/...]]` notes and `[[02.wiki/entities/...]]` / `[[02.wiki/projects/...]]` notes the decision touches. Cross-link from today's daily note.
...
**Anti-fabrication:** If you use advisor notes, base each verdict on what that note actually says - do not invent an advisor's position or attribute views to a real person they have not expressed. Never fabricate a consensus; report the real split. See the anti-fabrication and search-completeness hard rules in `.codex/references/ai-first-rules.md`.

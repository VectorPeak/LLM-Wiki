...
4. **Pick the destination.** Write under the project's hub: `02.wiki/projects/<name>/Architecture/` (create it if missing). If the vault has no project note for this codebase yet, offer to create one via `/obsidian-project` first so the architecture links into it.

...
3. Read or fetch the full source content:
   ...
   **For audio files** (.m4a, .mp3, .wav, .ogg, .webm):
   ...
   Save the transcript to `01.raw/12.Transcripts/`.
   ...
   **For images/screenshots** (.png, .jpg, .jpeg, .webp):
   ...
   Save the image description to `01.raw/01.Inbox/` as a markdown summary with context unless the user names a more specific raw folder.
...
5. Save the raw source to the matching `01.raw/` folder (immutable - never modify after saving):
   - Articles, pasted text, and uncertain sources: `01.raw/01.Inbox/YYYY-MM-DD - Source Title.md`
   - Website captures: `01.raw/07.Website/YYYY-MM-DD - Source Title.md`
   - Transcripts/audio: `01.raw/12.Transcripts/YYYY-MM-DD - Source Title.md`
   - Video-derived sources: `01.raw/13.Videos/YYYY-MM-DD - Source Title.md`
   - Papers and PDFs: use `01.raw/08.Research/` for research papers, otherwise use `01.raw/09.Book&Courses/` only when explicitly requested
...
6. **REWRITE the vault** - this is the critical step. Don't just create new pages. Rewrite existing ones.
   ...
   - **Entities agent**: for each person/company/tool mentioned:
     - Search `02.wiki/entities/` for existing page
   ...
   - **Concepts agent**: for each idea/framework/methodology:
     - Search `02.wiki/concepts/` for existing or related pages
   ...
   - **Projects agent**: for each project referenced:
     - Search `02.wiki/projects/` for matching project
...
7. Update structural files:
   ...
   - Append to the operation log: write `**HH:MM** - ingest | Source Title (type) - X created, Y rewritten, Z contradictions resolved` to `Logs/YYYY-MM-DD.md` (create `Logs/` and the daily file if missing; see `.codex/references/log.md` for the log format)

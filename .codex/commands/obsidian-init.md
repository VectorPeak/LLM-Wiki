---
description: Scan your vault and generate a _CLAUDE.md operating manual, index.md catalog, and operations-log reference
category: meta
...
2. Spawn parallel subagents to discover vault context simultaneously:
   ...
   - **Templates agent**: read all files in `03.templates/`
...
5. Generate `index.md` at the vault root - a catalog of all pages organized by category:
   - List every note in the vault grouped by folder (`01.raw/`, `02.wiki/projects/`, `02.wiki/entities/`, `02.wiki/concepts/`, etc.)
...
6. Initialize the vault operations log:
   ...
   - Write `.codex/references/log.md` as the operations-log reference: explain the per-day `Logs/YYYY-MM-DD.md` structure and include the entry template (do NOT put log entries in the reference file itself)
...
7. Create `Bases/` at the vault root if it does not exist. Stamp the four premade base files from `~/.codex/skills/obsidian-second-brain/references/bases/`:
   ...
   |---|---|---|---|
   | `projects.base.template` | `Bases/Projects.base` | `Projects` | `02.wiki/projects` |
   | `people.base.template` | `Bases/People.base` | `People` | `02.wiki/entities` |
   | `tasks.base.template` | `Bases/Tasks.base` | `Tasks` | `02.wiki/tasks` |
   | `daily.base.template` | `Bases/Daily.base` | `Daily` | `02.wiki/daily` |

   Detect vault style from the folder structure discovered in step 1: if `02.wiki/` exists at the root, use wiki-style folder names; otherwise use obsidian-style. For each template, replace the `{{FOLDER}}` placeholder with the correct folder name, then write to `Bases/`.

   Skip any base file that already exists in `Bases/` - never overwrite.

8. Write `_CLAUDE.md`, `index.md`, `.codex/references/log.md` (reference), `Logs/YYYY-MM-DD.md` (today's entries), and any new `Bases/*.base` files
...
If a legacy monolithic root log already exists with `## YYYY-MM-DD` sections: run `python ~/.codex/skills/obsidian-second-brain/scripts/migrate_log.py --vault <vault-path>` to split it into `Logs/YYYY-MM-DD.md` files. Do not overwrite manually.

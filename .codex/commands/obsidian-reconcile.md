...
3. Spawn parallel subagents to find contradictions:
   - **Claims agent**: scan `02.wiki/concepts/` and `02.wiki/projects/` for factual claims - find pairs that contradict each other
   - **Entity agent**: scan `02.wiki/entities/` for outdated roles, companies, or descriptions that conflict with newer sources
   - **Decisions agent**: scan `02.wiki/decisions/` and project Key Decisions for reversed or superseded decisions that were never updated
   - **Source freshness agent**: compare `01.raw/` source dates against `02.wiki/` page dates - flag wiki pages that reference old sources when newer ones exist on the same topic
...
5. Resolve each contradiction:
   - **Clear winner**: rewrite the outdated page with current info. Add a `## History` section noting what changed and why: "Previously stated X (source: 01.raw/07.Website/old-article.md, 2025-11-01). Updated to Y based on newer evidence (source: 01.raw/07.Website/new-article.md, 2026-03-15)."
   - **Genuinely ambiguous**: create `02.wiki/decisions/Conflict - Topic.md` documenting both sides, the evidence for each, and mark as `status: open` for the user to decide
...
6. After all resolutions:
   ...
   - Append to the operation log: write `**HH:MM** - reconcile | X contradictions found, Y auto-resolved, Z flagged for user` to `Logs/YYYY-MM-DD.md` (create `Logs/` and the daily file if missing; see `.codex/references/log.md` for the log format)

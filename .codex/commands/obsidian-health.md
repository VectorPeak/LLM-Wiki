...
4. Spawn parallel subagents to handle each category simultaneously:
   ...
   - **Contradictions agent**: scan Key Decisions sections and 02.wiki/concepts/ and 02.wiki/projects/ notes for claims that conflict with each other or have been superseded by newer sources
   ...
   - **Stale claims agent**: compare 02.wiki/concepts/ and 02.wiki/projects/ notes against their source dates - flag any note older than 6 months that references fast-moving topics (tools, APIs, pricing, team structure)
...
8. Append to the operation log: write `**HH:MM** - health | X critical, Y warnings, Z info` to `Logs/YYYY-MM-DD.md` (create `Logs/` and the daily file if missing; use `.codex/references/log.md` only for the log format)

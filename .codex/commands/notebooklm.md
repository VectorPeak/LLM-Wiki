...
3. The script does the whole flow end-to-end:
   ...
   - Writes the AI-first synthesis to `04.output/Research/NotebookLM/YYYY-MM-DD - <slug>.md`.
...
4. **After save, do the propagation step.** Same flow as `/research-deep`:
   ...
   - Run the standard `/obsidian-save` flow: spawn parallel subagents (People, Projects, Tasks, Decisions, Concepts/ideas) and update vault notes per any "Recommended next reads or angles" bullets if they map to entities, projects, or concepts.
   - Link the new synthesis note from today's daily note.

5. Report back to the user: "Saved [[YYYY-MM-DD - <slug>]] to 04.output/Research/NotebookLM/. Linked from today's daily note. Updated [[X]], created [[Y]]."
...
**AI-first rule:** Every note created or updated by this command MUST follow `.codex/references/ai-first-rules.md`. The saved synthesis at `04.output/Research/NotebookLM/YYYY-MM-DD - <slug>.md` follows the template baked into the script (preamble, frontmatter, vault-baseline links, response verbatim). Do not strip those.

# CC_runtime_05d.AgentTool实现详解

> 05c 中我们用 3 行介绍了 AgentTool“生成子 Agent，三种模式”。但 AgentTool 是所有工具中最复杂的一个。它是唯一一递归调用 `query_loop` 的工具，涉及 3 个文件、两种执行模式、一套隔离机制。本篇逐层展开。
...
为了防止无限递归（子 Agent 再创建子 Agent 再创建子 Agent...），子 Agent 的工具注册表中排除了 AgentTool 自身。这是一个硬性限制，写在 `execute()` 的 registry 构建逻辑中。
...
- `prompt` 是唯一必需参数（第 140 行做了空值检查）
- `model` 可选，未指定时使用构造函数传入的默认模型（第 57 行 `self._model`，默认 `claude-sonnet-4-20250514`）
- `run_in_background` 决定前台还是后台执行
- `isolation` 目前只支持 `"worktree"` 一个枚举值
...
在 Swarm（Teammate）模式下，排除规则更严格 —— 还会移除 `TeamCreate` 和 `TeamDelete`，确保只有 Leader 能创建 / 解散团队。不过这部分逻辑不在当前三个文件中，此处仅提及。
...
注意这里使用的是延迟导入（第 132 行 `from cc.core.query_loop import query_loop`），因为 `agent_tool.py` 和 `query_loop.py` 之间存在循环依赖：`query_loop` 通过 `ToolRegistry` 调用工具，而工具中包含 AgentTool，而 AgentTool.execute() 又调用 `query_loop`。延迟导入打破了这个模块级循环。
...
`finally` 块保证即使子 Agent 崩溃，worktree 也会被清理。`cleanup_agent_worktree` 本身的异常被单独捕获，清理失败不影响结果返回。
...
- `cc/tools/agent/agent_tool.py`，第 194-221 行
- `cc/tools/agent/background.py`

后台模式的核心区别是不阻塞父 Agent。子 Agent 的执行被封装为协程，交给 `BackgroundAgentManager` 管理，`execute()` 立即返回。
...
这个闭包捕获了所有准备好的上下文（`prompt`、`system_prompt`、`child_registry`、`call_model`、`_bg_perm`），定义了子 Agent 的完整执行逻辑。注意与前台模式的两个区别：

1. 没有 `TurnComplete` 的提前 `break` 检查 —— 后台 Agent 总是运行到 `query_loop` 自然结束
2. 没有 `try/except` —— 异常处理由 `BackgroundAgentManager._run_and_notify()` 统一负责
...
- `CancelledError`（任务被外部取消，如用户中断）只记录日志，不放入结果队列
- 其他异常将错误信息放入结果队列（`f"Agent error: {e}"`），并更新状态为 `FAILED`

TaskRegistry 状态更新有一个实现细节值得注意（第 94-97 行）：因为 TaskRegistry 的主键是 `task_id`（由 registry 自动生成，可能与 `agent_id` 不同），代码需要遍历所有记录，通过 metadata 中的 `agent_id` 字段匹配找到对应记录。这是一个 O(n) 查找，但后台 agent 数量通常很少，不构成性能问题。
...
主 REPL 循环在每次对话轮次开始前调用 `poll_completed()`，将后台 Agent 的完成结果注入到对话上下文中，让模型知道哪些后台任务已完成及其结果。
...
`git status --porcelain` 输出机器可读的状态。如果有输出，说明 worktree 中有未提交的修改。此时保留 worktree 不删除，只记录警告。这是安全策略 —— 如果子 Agent 修改了文件但还没来得及提交（或者 Agent 崩溃了），删除 worktree 会导致这些修改永久丢失。
...
使用 `git worktree remove` 而非直接删除目录。这是因为 git 在 `.git/worktrees` 中维护 worktree 的注册信息，直接删目录会留下悬空记录。删除失败不抛异常，只记录警告。
...
- `finally` 保证即使子 Agent 异常也会尝试清理
- 内层 `try/except` 保证清理自身的异常不影响结果返回

局限性：worktree 只隔离 git 管理的文件。非 git 文件（如临时文件写到 `/tmp` 其他位置）、网络请求、进程操作并不在隔离范围内。
...
`agent_tool.py` 是入口和编排者，负责参数处理、registry 裁剪、模式分支。`background.py` 负责后台任务的生命周期管理 —— 启动、追踪、通知。`worktree.py` 负责文件系统级隔离的创建和清理。三者职责清晰，通过 `execute()` 中的条件分支协同。

## OCR 不确定处

1. 第 1 张图顶部的参数提取代码块和右侧若干短注释已按可见内容重建；个别行号旁的补充说明做了保守转写。
2. 第 2 张图中的“模式 / 排除的工具 / 原因”表格按截图内容重建，Swarm 模式那一行的解释性尾句较长，现按可见信息归纳。
3. 第 4 张图后台模式 `_run_agent()` 代码块右侧注释较密，现保留关键差异点：无 `TurnComplete` 提前 break、无 `try/except`、由 `BackgroundAgentManager` 统一处理异常。
4. 第 7 张图最后的三文件协作全景本身就是 ASCII 调用图，正文保留为代码块，没有强制改写成 Mermaid，以避免结构失真。

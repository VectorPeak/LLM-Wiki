...
- [0. `cc/models/` 数据层](#0-ccmodels-数据层)
- [1. 先从 `Message` 和 `ContentBlock` 分层开始](#1-先从-message-和-contentblock-分层开始)
- [2. `Message` 层：谁在说话](#2-message-层谁在说话)
- [3. `ContentBlock` 层：说了什么 / 要做什么](#3-contentblock-层说了什么--要做什么)
- [4. 为什么要用 block，而不是直接拼字符串](#4-为什么要用-block而不是直接拼字符串)
- [5. `normalize_messages_for_api()` 是整个协议层的守门员](#5-normalize_messages_for_api-是整个协议层的守门员)
- [6. 为什么“工具协议正确性”比“输出文本正确性”更重要](#6-为什么工具协议正确性比输出文本正确性更重要)
- [7. 工具系统的最小抽象](#7-工具系统的最小抽象)
- [8. `ToolRegistry` 为什么是核心对象](#8-toolregistry-为什么是核心对象)
- [9. 模型和工具的关系不是“调用函数”，而是“协商协议”](#9-模型和工具的关系不是调用函数而是协商协议)
...
关键特征是：这个文件夹不依赖任何其他 `cc/` 子包（零 import），但几乎所有其他模块依赖它。
...
State（状态机）
  -> messages[]（谁在说话）
  -> ContentBlock[]（说了什么 / 要做什么）
```

这也是右侧总览图想表达的核心：`cc/models/` 是“三层数据结构”，像事件、借封、仪表盘一样把协议与状态拆开。

```mermaid
%%{init: {"theme": "forest", "flowchart": {"nodeSpacing": 24, "rankSpacing": 34, "padding": 16, "htmlLabels": true, "curve": "basis"}}}%%
flowchart LR
    A["State<br/>状态机变量"] --> B["messages[]<br/>谁在说话"]
    B --> C["ContentBlock[]<br/>说了什么 / 要做什么"]
```
...
`SystemMessage` 和 `CompactBoundaryMessage` 是两种“隐形信封”：

- 它们存在于 `messages[]`
- 但不会直接按原样发给 Claude API
- 会在 `normalize_messages_for_api()` 阶段被转换成协议可接受的形式
...
如果某个 `tool_result` 的 `tool_use_id` 在 `tool_use_ids` 中找不到对应项，直接删掉。  
如果删除后用户消息变成空 content，会填充占位文本 `"(no content)"`。
...
这是整个 transcript 修复逻辑里最关键的部分，保证了 API 调用前 `tool_use` 和 `tool_result` 始终成对。

### 对比总结

一轮传输前的 normalize 行为，可以这样理解：

| 类型 | normalize 之后保留 | Claude 看得到吗 | 存在 JSON 协议字段吗 | 用途 |
| --- | --- | --- | --- | --- |
| `UserMessage` | 保留 | 能看到 | 有 | 用户输入 / 工具反馈 |
| `AssistantMessage` | 保留 | 能看到 | 有 | 模型意图 |
| `SystemMessage` | 转成内部约束，不原样输出 | 不能直接看到 | 无直接消息体 | 提示词层 / 系统规则 |
| `CompactBoundaryMessage` | 转成摘要 user 消息 | 能看到摘要 | 有 | 历史压缩边界 |

## 6. 为什么“工具协议正确性”比“输出文本正确性”更重要

对 agent 来说，如果普通文本少一句话，通常问题不大。

但如果下面任一条件坏掉，整个回合都可能失败：
...
此外还有一个 `text` 属性（`@property`），作用是：无论 `content` 是字符串还是 block 列表，都能提取出纯文本表示。  
当 `content` 是列表时，它会遍历每个 block 取 `text` 字段拼接。

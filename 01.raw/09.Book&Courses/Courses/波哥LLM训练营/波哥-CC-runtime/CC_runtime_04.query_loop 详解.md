# CC_runtime_04.query_loop 详解

## 目录

- [1. 一句话定义](#1-一句话定义)
- [2. 架构角色：它为什么是整个 runtime 的心脏](#2-架构角色它为什么是整个-runtime-的心脏)
- [3. 完整状态机：一次完整 Query 的执行道路](#3-完整状态机一次完整-query-的执行道路)
- [4. 先看输入参数的语义](#4-先看输入参数的语义)
- [5. 局部状态变量其实就是算法骨架](#5-局部状态变量其实就是算法骨架)
- [6. Phase 1：规范化 transcript（Message）](#6-phase-1规范化-transcriptmessage)
- [7. Phase 2：auto-compact 判定](#7-phase-2auto-compact-判定)
- [8. Phase 3：调用模型并消费事件流](#8-phase-3调用模型并消费事件流)
- [9. Phase 4：流式归并](#9-phase-4流式归并)
- [10. Phase 5：错误恢复不是补丁，而是主流程的一部分](#10-phase-5错误恢复不是补丁而是主流程的一部分)
- [11. Phase 6：为什么工具结果要被包装成新的 `UserMessage`](#11-phase-6为什么工具结果要被包装成新的-usermessage)
- [12. `stop_reason` 不是绝对可信的](#12-stop_reason-不是绝对可信的)
- [13. 抽象成伪代码最容易看懂](#13-抽象成伪代码最容易看懂)
- [14. `query_loop()` 实际修改全局状态的时机只有 6 个](#14-query_loop-实际修改全局状态的时机只有-6-个)
- [15. 为什么它能既 streaming，又保持 transcript 一致性](#15-为什么它能既-streaming又保持-transcript-一致性)
- [16. 为什么 `TurnComplete` 会被 yield 两次语义上不同的场景](#16-为什么-turncomplete-会被-yield-两次语义上不同的场景)
- [17. 对算法工程师最有价值的抽象](#17-对算法工程师最有价值的抽象)
- [18. 这一篇最适合怎么配合源码读](#18-这一篇最适合怎么配合源码读)
...
`query_loop` 是整个系统的“心脏”，一个 `while(true)` 状态机，驱动“调用模型 -> 解析响应 -> 执行工具 -> 拼回结果 -> 再次调用模型”的循环。

它是纯函数式的：**所有依赖都通过参数注入，不持有任何长期状态，长期状态在控制面**。这使得它可以被 `QueryEngine`、`AgentTool`、测试 mock 等多种场景复用。
...
- `query_engine.py`（唯一调用方）

## 3. 完整状态机：一次完整 Query 的执行道路

### 3.1 完整状态机总览

下面这张 Mermaid 不是原图逐像素复刻，而是把 9 张图里重复出现的主流程压成一张更适合 Obsidian 阅读的结构图。

```mermaid
%%{init: {"theme": "forest", "flowchart": {"nodeSpacing": 24, "rankSpacing": 34, "padding": 16, "htmlLabels": true, "curve": "basis"}}}%%
flowchart TB
    A["while turn_count < max_turns"] --> B["Phase 1<br/>规范化消息 + Auto-Compact"]
    B --> C["Phase 2<br/>调用 call_model() 并消费 QueryEvent"]
    C --> D{"有错误?"}
    D -->|Yes| E["Phase 3<br/>错误恢复 / 重试 / reactive compact"]
    E --> F{"可恢复?"}
    F -->|Yes| B
    F -->|No| Z["yield ErrorEvent<br/>return"]
    D -->|No| G["Phase 4<br/>归并 AssistantMessage"]
    G --> H{"有 tool_use_blocks?"}
    H -->|No| Y["return<br/>正常结束"]
    H -->|Yes| I["执行工具并等待结果"]
    I --> J["把 ToolResultBlock 包装成 UserMessage"]
    J --> K["append 到 transcript"]
    K --> L["turn_count += 1<br/>进入下一轮"]
    L --> B
```

### 3.2 一次完整 Query 的执行道路

一次完整 Query 的执行道路可以理解为：

1. 初始化 `engine.run_turn()` 进入 `query_loop`
2. 进入 `while turn_count < max_turns`
3. `Phase 1` 规范化消息并判断是否触发 auto-compact
4. `Phase 2` 流式调用模型，消费统一的 `QueryEvent`
5. 流式过程中收集：
   - `accumulated_text`
   - `usage`
   - `stop_reason`
   - `tool_use_blocks`
   - `error_event`
6. 若有错误则进入 `Phase 3` 恢复分支
7. 若无错误则 materialize 本轮 assistant message
8. 如果没有工具调用，直接结束本轮并 return
9. 如果有工具调用，执行工具并等待结果
10. 把工具结果包装成新的 `UserMessage`
11. 写回 transcript，进入下一轮
...
## 6. Phase 1：规范化 transcript（Message）
...
这一步的职责非常关键：把“内部历史”变成“模型 API 能安全接受的历史”。
...
这一层的关键不是“调模型”，而是：`query_loop()` 完全不关心底层 SSE 细节，它只消费统一的 `QueryEvent`。
...
- 未来替换 provider 时，`query_loop` 基本不用动
...
这体现的是：长上下文溢出优先用 transcript 变换来恢复，而不是立刻报错。

### 10.2 `max_output_tokens`
...
## 11. Phase 6：为什么工具结果要被包装成新的 `UserMessage`
...
这意味着工具执行本质上不是副作用而已，而是：**一次会话协议中的显式消息交换**。

## 12. `stop_reason` 不是绝对可信的
...
- `stop_reason` 可能不稳定
- 但 transcript 中有没有 `tool_use_block` 才是更坚实的事实
...
## 14. `query_loop()` 实际修改全局状态的时机只有 6 个
...
## 15. 为什么它能既 streaming，又保持 transcript 一致性
...
## 16. 为什么 `TurnComplete` 会被 yield 两次语义上不同的场景
...
理解成这组逻辑之后，你再去想如何替换规划器、改动作空间、接记忆系统，都会清楚很多。

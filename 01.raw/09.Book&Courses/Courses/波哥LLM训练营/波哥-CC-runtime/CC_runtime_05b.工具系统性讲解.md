...
flowchart TB
    A["ToolUseBlock"] --> B["ToolRegistry<br/>查找工具"]
    B --> C["StreamingToolExecutor<br/>调度执行"]
    C --> D["PreToolUse Hook<br/>是否拦截"]
    D --> E["Permission Gate<br/>是否允许"]
    E --> F["tool.execute()<br/>实际执行"]
    F --> G["PostToolUse Hook<br/>事后通知"]
    G --> H["ToolResult"]
```
...
`ToolSchema`（第 22-32 行）是工具向 API 注册的描述结构，包含三个字段：

- `name`：唯一标识符
- `description`：功能描述，影响模型是否选择调用
- `input_schema`：JSON Schema 格式的参数定义

这个字段直接对应 Anthropic API 请求中 `tools` 数组的每个元素。

### ToolResult

`ToolResult`（第 35-66 行）是所有工具执行后的统一返回格式。`content` 字段支持两种类型：

- `str`：绝大多数工具
- `list[dict]`：富内容，如 `FileReadTool` 读取图片时返回 base64 内容块

`is_error` 标记结果是否为错误。这是一个关键设计决策：工具错误不抛异常，而是返回 `is_error=True` 的 `ToolResult`，这样 query loop 不会中断，模型可以看到错误信息并自主决定重试、换参数还是向用户解释。

`.text` 属性提供统一的文本提取接口，当 `content` 是富内容列表时从每个 dict 中提取 `"text"` 字段并用换行连接。
...
- `get_name()`：工具名，API 层面的唯一标识
- `get_schema()`：JSON Schema 描述
- `execute()`：执行逻辑
- `is_concurrency_safe()`：默认返回 `False`

## 2. ToolRegistry：工具的注册与发现

`ToolRegistry`（`cc/tools/base.py` 第 115-161 行）是工具发现机制的核心，用 `dict[str, Tool]` 存储，保证 `O(1)` 查找。
...
- Tier 2（lazy import）：`TaskTools`、`WebFetchTool`、`WebSearchTool`、`NotebookEditTool`、`SkillTool`、`PlanModeTool`、`BriefTool`、`LSPTool`、`ToolSearchTool`、`AskUserQuestionTool`
- Tier 3（协作工具）：`AgentTool`、`TeamCreateTool`、`TeamDeleteTool`、`SendMessageTool`、`TodoWriteTool` —— 需要 `call_model_factory` 等运行时依赖，在 `_build_engine()` 中做二次布线

## 3. StreamingToolExecutor：流式提前执行【核心优化机制之一】

这是整个工具系统最核心的创新点，定义在 `cc/tools/streaming_executor.py`。

传统方案等 API 流式响应完全结束后才开始执行工具。假设模型返回 3 个 `tool_use`，传统模式需要等 3 个都解析完才启动第 1 个。`StreamingToolExecutor` 在第 1 个 `tool_use block` 完整解析出来时就立即启动执行，第 2、3 个到达时第 1 个可能已经完成了。

### 触发时机

`query_loop.py` 第 166-196 行展示了集成方式。在 Phase 2 的流式循环中，每收到一个 `ToolUseStart` 事件（对应 API 流的 `content_block_stop`），就调用 `executor.add_tool(block)` 将工具交给执行器。流结束后调用 `executor.get_results()` 收集所有结果。
...
- `_pending`：记录所有已启动的 `(tool_use_id, asyncio.Task)` 对，用于最终收集结果
- `_queue`：暂存不能立即执行的工具
- `_has_exclusive_running`：标记当前是否有非并发安全工具在独占执行，一为 `True` 即使是并发安全的工具也必须排队
- `_semaphore = asyncio.Semaphore(MAX_CONCURRENCY)`：并发限流（10）
...
排队的原因不仅是工具自身不安全。如果当前 `Edit` 正在独占执行，即使是 `Read` 也不能并行，因为可能读到 `Edit` 写了一半的文件内容。这是注释中明确指出的竞态条件防护（第 93-96 行）。

### 3.3 _execute_one() 的四层执行栈

单个工具的完整执行流程在 `_execute_one()`（第 105-146 行），可层层拆成：

1. `Semaphore` 限流：`async with self._semaphore` 确保并发数不超过 `MAX_CONCURRENCY=10`，防止大量并发文件 / I/O / 耗时文件描述符。
2. `PreToolUse Hook`：如果配置了 hooks，调用 `run_pre_tool_hooks()` 检查是否有 hook 阻止执行。被阻止时直接返回 `is_error=True` 的 `ToolResult`，工具的 `execute()` 根本不会被调用。这就是 Hook 的“短路”能力。
3. `Permission Check`：权限检查器（`self._permission_checker`）判断用户是否授权。在 `ACCEPT_EDITS` 模式下，`Bash` 这类高危工具需要用户确认，未授权则返回 `"Denied by permission policy"`。
4. 实际执行：`await tool.execute(block.input)` 调用工具的具体实现，用 `try/except` 包裹，异常转为 `ToolResult(content=f"Error: {e}", is_error=True)`，保证单个工具崩溃不中断整个 `query_loop`。
...
也就是先控所有已启动的并发任务跑完，再启动独占工具，再等它完成，最后清除独占标记，继续处理队列下一个。
...
(注：此时没有独占锁，但 Edit 已入队，后续的 safe 工具不受影响，
  但 Edit 之后的工具会在 _process_queue 中被重新排序)
...
它只是简单地看两个列表是否非空，并不区分 Task 是否真的还在跑。

也就是说，即使某个 Task 已经完成了，只要还没被 `get_results()` 消费，它仍然在 `_pending` 里，`has_pending` 仍然返回 `True`。

### 3.8 但这样不会误判吗

不会，因为 `has_pending` 的唯一用途是让 `query_loop` 判断要不要调用 `get_results()`。
...
has_pending == True -> 调用 get_results()
    ↓
get_results() 内部 await 所有 task，收集结果，_pending 被清空
    ↓
...
`cc/tools/orchestration.py` 中有一个 `run_tools()` 函数，采用“流结束后一次性批量执行”的方案。当前代码中没有任何地方调用它 —— `query_loop.py:169` 只使用 `StreamingToolExecutor`。它作为未启用的备选方案存在，不需要深入了解。（`StreamingToolExecutor` 的下位替代）
...
`HookConfig`（第 30-41 行）包含三个字段：

- `event`（`"PreToolUse"` 或 `"PostToolUse"`）
- `command`（shell 命令）
- `tool_name`（可选，限定只对特定工具生效）

配置来源是 `~/.claude/settings.json` 的 `hooks` 字段。

### 执行协议

`run_hook()`（第 97-155 行）通过 `asyncio.create_subprocess_shell` 启动子进程，工具上下文以 JSON 格式通过 `stdin` 传入，退出码决定行为：
...
`run_pre_tool_hooks()`（第 158-183 行）：按注册顺序依次执行所有匹配的 hook，只要有一个返回 `blocked=True` 就立即停止后续 hook 并返回阻止结果。

### PostToolUse 的语义

`run_post_tool_hooks()`（第 186-209 行）：在工具完成后运行，无法阻止执行（结果已产生）。工具输出截断到 1000 字符传递给 hook，避免大量输出撑爆 hook 进程的内存。返回值为 `None`，因为 post hook 的结果不影响流程。
...
效果：帮助清理临时文件时可以放行，但遇到危险删除命令会被 PreToolUse 直接拦住。
...
flowchart TB
    A["模型决定调用<br/>Bash(command='rm -rf /tmp/cache/')"] --> B["PreToolUse Hook 启动"]
    B --> C["stdin: 传入 tool_name + input.command"]
    C --> D["check_safety.py 检查<br/>发现 rm -> exit(2)"]
    D --> E["工具被阻止<br/>返回 ToolResult(is_error=True)"]
    E --> F["模型看到工具被阻止<br/>改换方式或解释原因"]
```

对应 `_execute_one(block)` 中的位置：
...
> Permission 是系统自带的粗粒度开关（按工具名），Hook 是用户自定义的细粒度检查（按具体参数）。
...
`check_permission()`（第 64-94 行）的判定流程是：

- `BYPASS` 模式一律 `ALLOW`
- 只读工具一律 `ALLOW`
- 编辑工具在 `ACCEPT_EDITS` 模式下 `ALLOW`
- 其余返回 `ASK`

`Bash`、`Agent`、`WebFetch` 等工具不在任何白名单中，始终需要确认。

### PermissionContext

`PermissionContext`（第 97-206 行）是会话级的权限状态容器。`check()` 方法的完整流程：

- 检查 `_always_allow` 缓存
- 检查自定义 rules
- 执行模式检查
- 如果是 `ASK` 且非交互模式则直接拒绝（第 162-166 行）

非交互模式包括 `--print` 管道模式和后台 agent，这些场景无法向用户弹窗确认，宁可拒绝也不能在无人监督时执行高危操作。

`_always_allow` 缓存（第 120 行）存储用户选择 `"a"`（always）后的工具名。用户在交互确认时选 `"a"` 后，该工具在当前会话中的后续调用都会自动放行，不再弹窗。
...
`cc/skills/loader.py` 第 22-35 行定义：
...
Markdown 文件，支持可选的 YAML frontmatter。`_parse_skill_file()`（第 69-117 行）用正则：
...
`load_skills()`（第 38-66 行）：

- `~/.claude/skills/`（用户级，跨项目共享）
- `.claude/skills/`（项目级，随代码分发）
...
2. 模型工具调用：模型调用 `SkillTool`（`cc/tools/skill/skill_tool.py`），`execute()` 查找对应 skill 并将 prompt 文件作为 `ToolResult.content` 返回

两条路径最终效果相同 —— skill 的 Markdown prompt 进入了对话上下文。

`SkillTool` 的 `is_concurrency_safe` 返回 `True`（第 60-62 行），因为它只是内存中的字符串查找，没有任何副作用。
...
1. 第 1 张图中的“05 TOOL 编排与执行”总览图信息密度很高，正文已转成主执行链 Mermaid，并将各子卡片拆成文字说明，没有逐项抄录图中所有细小标签。
2. 第 2 张图中的“三层注册 + 二次布线”架构板块主要转写成分层列表；图内某些行号级别标注（如 `main.py:xx-yy`）未逐一展开。
3. `3.5` 的时序示例中，截图注释对“Edit 入队后后续 safe 工具受影响”的措辞较密，这里按可见语义保守重建。
4. `5. Hook 系统` 里的真实场景截图底部“效果”区域只看到“帮我清理临时文件”等局部文字，未继续外推更多展示内容。

...
flowchart TB
    subgraph OUTER["外层: 控制面 / shell"]
        ...
        M4["注册工具"]
        M5["管理 REPL 生命周期"]
        M6["管理 session / history / memory extraction"]
        M --> M1
        M --> M2
        M --> M3
        M --> M4
    ...
    subgraph INNER["内层: agent runtime"]
        Q["cc/core/query_engine.py<br/>QueryEngine"]
        L["cc/core/query_loop.py<br/>query_loop()"]
        T["把 transcript 发给模型"]
        E["接收流式事件"]
        U["识别 tool_use"]
        X["执行工具"]
        R["把 tool_result 塞回 transcript"]
        C["做错误恢复和 compact"]
        P["管理权限门控<br/>(permission checker)"]
        Q --> L
        L --> T
        L --> E
        L --> U
...
flowchart TB
    A["Phase 0<br/>启动阶段"] --> B["Phase 1<br/>用户输入"]
    B --> C["Phase 2<br/>构造 UserMessage<br/>追加到 messages"]
    C --> D["Phase 3<br/>query_loop 第一次发送 Claude API"]
    D --> E["Phase 4<br/>API 流式返回<br/>文本增量 + 工具调用"]
    E --> F["Phase 5<br/>若需要工具则执行"]
    F --> G["Phase 6<br/>塞回一个新输入<br/>messages 状态变更"]
    G --> H["Phase 6'<br/>query_loop 再跑一圈"]
    H --> I["Phase 7<br/>后处理: save_session / add_to_history / extract_memories"]
```
...
| --- | --- |
| `_run_repl()` | 交互式运行主循环 |
| `_build_engine()` | 组装 QueryEngine 及其全部依赖 |
...
async def _run_repl(model: str, resume_id: str | None = None) -> None:
    ...
    【初始化阶段】
      ...
      - 如果有 resume_id，加载并修复 transcript + task snapshot
...
if resume_id:
    ...
    if loaded:
        ...
        # 半途终止后（RUNNING/PENDING）会被标记为 KILLED，因为对应的
        # asyncio.Task 已丢失
...
while True:
    ...
    if user_input.strip().startswith("/"):
        ...
        if result == "__CLEAR__":
            ...
            engine.system_prompt = build_system(cwd, engine.model, claude_md)
...
### 普通用户输入 + team 收件箱 + query_loop 驱动

```python
else:
    # 普通用户输入 -> 构造 UserMessage 追加到 transcript
    # 这使 transcript 的第一个写入点：用户消息落盘
...
```mermaid
%%{init: {"theme": "forest", "sequence": {"mirrorActors": false, "messageAlign": "left"}}}%%
sequenceDiagram
...
| 层 | 职责 |
| --- | --- |
| `main.py` | 生命周期管理、prompt 组装、registry 装配、REPL/CLI UI、slash command 分流、session/history/memory 的后处理 |
| `query_loop.py` | 单次 agent 运行、transcript 到 API 的推进、事件流到状态的归并、工具回环、错误恢复、compact 触发 |
| `tools/*` | 真正执行本地操作 |
| `api/claude.py` | 把 Claude 流式 API 映射成统一事件 |
...
| --- | --- |
| `TextDelta` | 流式文本增量 |
| `ThinkingDelta` | 流式 thinking 文本增量 |
| `ToolUseStart` | 工具调用发起 |
| `ToolResultReady` | 工具执行完成 |
| `CompactOccurred` | 上下文已被压缩 |
| `TurnComplete` | 本轮结束 |
| `ErrorEvent` | 错误事件（可恢复 / 不可恢复） |
...
1. 第 1 张图中的“两层分工架构图”和第 7 张图中的“SYSTEM PROMPT 分层结构 + ENGINE 装配全景”都属于高信息密度总览图；正文已转成 Mermaid 和结构化说明，但没有逐项转录所有细小标注。
2. `4. 主链路先看 REPL` 中代码块按可见内容重建，保留了主要结构和关键语句；个别灰色注释和行号周边的极细碎说明未逐字展开。
3. `5. 一次用户输入的完整序列` 左侧原图是 `sequenceDiagram` 代码截图；这里按截图内容重建为 Mermaid，个别参与者注释略有压缩，以保证 Obsidian 内整体可读。
4. `7. 外层和内层的职责边界` 表格右侧最后一列在截图中靠近页面边缘，现按可见文字进行保守转写。
